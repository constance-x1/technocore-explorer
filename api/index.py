...
Ultra-low latency, concurrent, self-contained Vercel Serverless Function & API Gateway.
Zero external dependencies, zero relative import fragility.
"""
...
def scan_did_agent(did: str, base_url: str = DEFAULT_BASE_URL) -> dict[str, Any]:
    ...
    # Parallelize DID note fetch AND room scanning concurrently
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            matched_path = self.headers.get("x-matched-path", raw_path)
            combined_path = f"{raw_path} {matched_path}".lower()
            query = urllib.parse.parse_qs(parsed.query)

            # Route 1: Health Check
            if "health" in combined_path or "health" in query:
                self._send_json(200, {
                    "status": "healthy",
            ...
            # Route 2: Scan Request
            # Matched if 'did' query is given OR path contains 'scan'
            if "did" in query or "scan" in combined_path:
                target_did = ""
                if "did" in query and query["did"]:
                    target_did = query["did"][0].strip()
                elif "/scan/" in raw_path:
                    target_did = urllib.parse.unquote(raw_path.split("/scan/", 1)[1].strip())
                elif "/scan/" in matched_path:
                    target_did = urllib.parse.unquote(matched_path.split("/scan/", 1)[1].strip())

                if not target_did:
                    self._send_json(400, {
                        "status": "error",
                        "error": "Missing 'did' query parameter. Example: /api/scan?did=did:key:z6Mk..."
                    })
                    return
            ...
            # Route 3: Network Overview (Default for /api/overview or /api)
            ...
            self._send_json(500, {
                "status": "error",
                "error": f"Serverless execution error: {str(e)}"
            })
